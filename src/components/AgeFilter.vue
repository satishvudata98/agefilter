<template>
    <v-app>
      <v-container>
          <v-row>
              <v-col cols="12">
                <v-btn @click="addNewCard" color="primary" class="d-flex float-right">Add New Card</v-btn>
                <v-text-field v-model="search" label="Search by Name" style="width: 300px;"></v-text-field>
              </v-col>
            </v-row>
        <v-row>
          <v-col v-for="(range, index) in ageRanges" :key="index" cols="3">
            ...
            <v-row dense>
              <v-col
                ...
                cols="12"
              >
                <v-card  v-if="matchesSearch(item.name)" class="card-style" color="grey" dark>
